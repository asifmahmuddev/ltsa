    ...
    Object getMethod(String paramString, Class[] paramArrayOfClass) throws NoSuchMethodException {
        ...
        if (vector.size() == 0)
            throw new NoSuchMethodException(
                "Function \"" + paramString + "\" exists," + " but parameters " + describe(paramString, paramArrayOfClass) + " can not be accepted by it."
            );
        if (vector.size() == 1)
        ...
        while (enumeration2.hasMoreElements()) {
            ...
            if ((bool1 ^ bool2) == 0)
                throw new NoSuchMethodException(
                    "Ambiguity detected between \"" + describe(paramString, arrayOfClass) + "\" and \""
                    + describe(paramString, arrayOfClass1) + "\" on invocation \"" + describe(paramString, paramArrayOfClass) + "\" ."
                );
        }
        return object;
    }
