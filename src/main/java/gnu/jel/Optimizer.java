...
public class Optimizer {
    ...
    public void binaryOP(int paramInt, boolean paramBoolean) throws IllegalStateException {
        ...
        if (!paramBoolean) {
            if (!ExpressionImage.canGenerateBinary(paramInt, clazz2, clazz1)) {
                bool = true;
                str = "Types " + clazz2.toString() + " and " + clazz1.toString() + " are not suitable " + "for the operation \""
                    + ExpressionImage.binaryNames[paramInt] + "\"";
            }
        } else if (clazz2 != boolean.class || clazz1 != boolean.class) {
            bool = true;
            str = "Types " + clazz2.toString() + " and " + clazz1.toString() + " are not suitable for \"" + ExpressionImage.logicalNames[paramInt]
            ...
            if (oP instanceof OP_start && ((OP_start) oP).getFunction() == oP_conditional) {
                ...
                if (clazz3 == null)
                    throw new IllegalStateException(
                        "Operands of conditional have types " + clazz2.getName() + " and " + clazz1.getName()
    ...
    public void logical_not() throws IllegalStateException {
        if (this.types.peek() != boolean.class)
            throw new IllegalStateException(
                "You tried to use logical complement on " + ((Class) this.types.peek()).getName() + ". This operation is supported only"
                + " on booleans."
            );
