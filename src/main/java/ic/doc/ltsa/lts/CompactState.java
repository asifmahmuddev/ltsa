...
public class CompactState implements Automata {
    ...
    private static void copyOffset(int paramInt, EventState[] paramArrayOfEventState, CompactState paramCompactState, boolean paramBoolean) {
        for (byte b = 0; b < paramCompactState.states.length; b++) {
            if (!paramBoolean) {
                paramArrayOfEventState[b + paramInt] = EventState.offsetSeq(
                    paramInt, paramCompactState.endseq, paramCompactState.maxStates + paramInt, paramCompactState.states[b]
                );
            } else {
                paramArrayOfEventState[b + paramInt] = EventState.offsetSeq(
                    paramInt, paramCompactState.endseq, paramCompactState.endseq + paramInt, paramCompactState.states[b]
                );
            }
        }
    }
